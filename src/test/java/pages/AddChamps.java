...
public class AddChamps extends BaseTest {
    ...
    public String getErrorMessageForInvalidPhoneNumber() {
        return wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath(loc.getProperty("invalidPhoneNumberMessageXpath")))).getText();
    }

    // Method to get the error message for an invalid name
    public String getErrorMessageForInvalidName() {
        return wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath(loc.getProperty("invalidNameMessageXpath")))).getText();
    }
